...
final class SimpleUpdater {
	...
	SimpleUpdater(Plugin plugin, int projectId) {
		if (plugin == null) {
	 ...
	 */
	void checkForUpdates(final ResponseHandler responseHandler) {
		Thread updaterThread = new Thread(() -> {
            ...
            try {
                ...
                if (isNewerVersion(newVersion)) {
                    Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> responseHandler.onUpdateFound(newVersion));
                }
